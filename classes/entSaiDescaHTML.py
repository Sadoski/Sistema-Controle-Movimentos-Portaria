...
class TAG:
    ...
    def gerar(dados):
        ...
        head = HEAD(TITLE('Relatório Entrada e Saida Descarregamento'))
        ...
        body = BODY()
        body <= H1('Relatório Entrada e Saida Descarregamento')
        tab = TABLE()
        body <= tab
        thead = THEAD()
        tab <= thead
        ...
        for a in dados:
            ...
            trr <= TD(a[5])
            trr <= TD(a[6])
            trr <= TD(entSaiDao.pesquisarMotoristaDescaRel(a[7]))
            trr <= TD(a[8])
            trr <= TD(a[9])
            trr <= TD(a[10])
            trr <= TD(entSaiDao.pesquisarFornecedorDescaRel(a[11]))
